...
pub(crate) fn is_mutable_slot(
	...
	if meta.guard {
		...
		// `knows private k` declared by two principals is one slot with one
		// creator, so the other principal's copy has a foreign creator yet never
		// travelled.  The test is whether it crossed a wire at all, not who made
		// it — and deliberately not whether *this* principal received it, since
		// a value substituted on an earlier hop still reaches a later principal
		// (`exa.vp`).
		//
		// This hides attacks as well as inventing them: a recipient's own key
		// admitted as controllable enters the term as `$k`, so its rewrite rule
		// demands `ENC($k, ...)` and the forgery that key unlocks is never
		// proposed.
		return false;
	}
