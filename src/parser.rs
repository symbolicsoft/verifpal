...
fn check_reserved(s: &str) -> Result<(), String> {
    ...
    if RESERVED.contains(&lower.as_str()) || lower.starts_with("attacker") || lower.starts_with("unnamed") {
...
impl<'a> Parser<'a> {
    ...
    fn expect(&mut self, s: &str) -> Result<(), String> {
        ...
        if self.pos + bytes.len() <= self.input.len() && &self.input[self.pos..self.pos + bytes.len()] == bytes {
            self.pos += bytes.len();
            Ok(())
        } else {
    ...
    fn parse_identifier(&mut self) -> Result<String, String> {
        ...
        let s = std::str::from_utf8(&self.input[start..self.pos]).expect("identifier is valid UTF-8");
        Ok(s.to_lowercase())
    }
    ...
    fn parse_principal(&mut self) -> Result<Block, String> {
        ...
        Ok(Block {
            ...
            principal: Principal { name, id, expressions },
    ...
    fn parse_message_constants(&mut self) -> Result<Vec<Constant>, String> {
        ...
        loop {
            ...
            if rem.starts_with("principal") || rem.starts_with("phase") || rem.starts_with("queries") || rem.starts_with("//") {
                break;
            }
    ...
    fn parse_knows(&mut self) -> Result<Expression, String> {
        ...
        let qualifier = match qualifier_str.as_str() {
            "private" => TypesEnum::Private,
            "public" => TypesEnum::Public,
            "password" => TypesEnum::Password,
            _ => return Err(format!("invalid qualifier: {}", qualifier_str)),
        };
        self.skip_whitespace();
        let constants = self.parse_constants()?;
        Ok(Expression {
            kind: TypesEnum::Knows,
            qualifier,
            constants,
            assigned: None,
        })
    ...
    fn parse_constants(&mut self) -> Result<Vec<Constant>, String> {
        ...
        loop {
            ...
            if self.at_end() { break; }
            let c = self.peek();
            if c == Some(b'=') || c == Some(b']') || c == Some(b')') || c == Some(b'\n') || c == Some(b'\r') {
                break;
            }
            // Check for keywords that end the constant list
            let rem = self.remaining();
            if rem.starts_with("knows") || rem.starts_with("generates") || rem.starts_with("leaks") || rem.starts_with("//") {
                break;
            }
    ...
    fn parse_phase(&mut self) -> Result<Block, String> {
        ...
        let num_str = std::str::from_utf8(&self.input[start..self.pos]).expect("phase number is valid UTF-8");
        let number: i32 = num_str.parse().map_err(|_| "invalid phase number".to_string())?;
        self.skip_whitespace();
        self.expect("]")?;
        self.skip_whitespace_and_comments();
        Ok(Block {
    ...
    fn parse_query_confidentiality(&mut self) -> Result<Query, String> {
        ...
        let constant = match c { Value::Constant(c) => c, _ => return Err("expected constant".to_string()) };
        self.skip_whitespace();
        let options = self.try_parse_query_options()?;
        Ok(Query {
            kind: TypesEnum::Confidentiality,
    ...
    fn parse_query_constant_list(&mut self) -> Result<Vec<Constant>, String> {
        ...
        loop {
            ...
            let rem = self.remaining();
            if rem.starts_with("confidentiality") || rem.starts_with("authentication") ||
               rem.starts_with("freshness") || rem.starts_with("unlinkability") ||
               rem.starts_with("equivalence") || rem.starts_with("//") {
                break;
            }
    ...
    fn try_parse_query_options(&mut self) -> Result<Vec<QueryOption>, String> {
        ...
        while self.peek() != Some(b']') {
            ...
            let option_enum = match option_name.as_str() {
                "precondition" => TypesEnum::Precondition,
                _ => TypesEnum::Empty,
            };
            let sender_id = principal_names_map_add(&sender_name);
            let recipient_id = principal_names_map_add(&recipient_name);
