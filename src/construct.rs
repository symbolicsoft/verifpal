...
pub fn construct_knowledge_map(
    m: &Model, principals: &[String], principal_ids: &[PrincipalId],
    ...
    km.constant_index.insert(nil_const.id, km.constants.len() - 1);
    ...
    for blck in &m.blocks {
        match blck.kind.as_str() {
            "principal" => {
                let (new_km, new_da) = construct_knowledge_map_render_principal(
                    km, blck, declared_at, current_phase,
                )?;
                km = new_km;
                declared_at = new_da;
            }
...
fn construct_knowledge_map_used_by(km: &KnowledgeMap) -> HashMap<ValueId, HashMap<PrincipalId, bool>> {
...
fn construct_knowledge_map_render_principal(
    mut km: KnowledgeMap, blck: &Block, mut declared_at: i32, current_phase: i32,
...
fn construct_knowledge_map_render_knows(
    mut km: KnowledgeMap, blck: &Block, declared_at: i32, expr: &Expression,
) -> Result<KnowledgeMap, String> {
    for c in &expr.constants {
        ...
        if let Some(idx) = i {
            ...
            if d1 != TypesEnum::Knows || q1 != q2 || fresh {
                return Err(format!("constant is known more than once and in different ways ({})", pretty_constant(c)));
            }
            let mut m = HashMap::new();
        ...
        if expr.qualifier != TypesEnum::Public {
            continue;
        }
...
fn construct_knowledge_map_render_generates(
    ...
    for c in &expr.constants {
        let i = value_get_knowledge_map_index_from_constant(&km, c);
        if i.is_some() {
            return Err(format!("generated constant already exists ({})", pretty_constant(c)));
        }
        let new_c = Constant {
            name: c.name.clone(), id: c.id, guard: c.guard,
            fresh: true, leaked: false,
            declaration: TypesEnum::Generates, qualifier: TypesEnum::Private,
        };
        km.constants.push(new_c.clone());
        km.constant_index.insert(new_c.id, km.constants.len() - 1);
        km.assigned.push(Value::Constant(new_c));
...
fn construct_knowledge_map_render_assignment(
    ...
    for c in &constants {
        ...
        if !knows {
            return Err(format!("{} is using constant ({}) despite not knowing it",
                blck.principal.name, pretty_constant(c)));
        }
    }
    for (i, c) in expr.constants.iter().enumerate() {
        ...
        let new_c = Constant {
            name: c.name.clone(), id: c.id, guard: c.guard,
            fresh: false, leaked: false,
            declaration: TypesEnum::Assignment, qualifier: TypesEnum::Private,
        };
...
fn construct_knowledge_map_render_leaks(
    mut km: KnowledgeMap, blck: &Block, expr: &Expression, current_phase: i32,
) -> Result<KnowledgeMap, String> {
    for c in &expr.constants {
        let idx = match value_get_knowledge_map_index_from_constant(&km, c) {
            Some(idx) => idx,
            None => return Err(format!("leaked constant does not exist ({})", pretty_constant(c))),
        };
        ...
        if !known {
            return Err(format!("{} leaks a constant that they do not know ({})",
                blck.principal.name, pretty_constant(c)));
        }
...
fn construct_knowledge_map_render_message(
    mut km: KnowledgeMap, blck: &Block, current_phase: i32,
) -> Result<KnowledgeMap, String> {
    for c in &blck.message.constants {
        let idx = match value_get_knowledge_map_index_from_constant(&km, c) {
            Some(idx) => idx,
            None => return Err(format!("{} sends unknown constant to {} ({})",
        ...
        for m in &km.known_by[idx] {
            if m.contains_key(&blck.message.sender) { sender_knows = true; }
            if m.contains_key(&blck.message.recipient) { recipient_knows = true; }
        }
        if !sender_knows {
            return Err(format!("{} is sending constant ({}) despite not knowing it",
                principal_get_name_from_id(blck.message.sender), pretty_constant(c)));
        }
        if recipient_knows {
            return Err(format!("{} is receiving constant ({}) despite already knowing it",
                principal_get_name_from_id(blck.message.recipient), pretty_constant(c)));
        }
        let mut m = HashMap::new();
...
pub fn construct_principal_states(m: &Model, km: &KnowledgeMap) -> Vec<PrincipalState> {
    ...
    for p in 0..km.principals.len() {
        ...
        for (i, c) in km.constants.iter().enumerate() {
            ...
            for blck in &m.blocks {
                if blck.kind == "message" {
                    let (w, g, mt) = construct_principal_states_get_value_mutatability(
                        c, blck, km.principal_ids[p], km.creator[i], wire, guard, mutatable_to,
                    );
                    wire = w;
                    guard = g;
                    mutatable_to = mt;
...
fn construct_principal_states_get_value_mutatability(
    c: &Constant, blck: &Block, principal_id: PrincipalId, creator: PrincipalId,
    mut wire: Vec<PrincipalId>, mut guard: bool, mut mutatable_to: Vec<PrincipalId>,
) -> (Vec<PrincipalId>, bool, Vec<PrincipalId>) {
    if blck.kind != "message" { return (wire, guard, mutatable_to); }
