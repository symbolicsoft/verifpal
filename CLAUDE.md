...
**Fix engine and analysis bugs by keeping the design lean, not by piling fixes on top of each other.** If the fix is simple, make the simple fix. Otherwise, do not wrap a flawed component in another layer, special case or side table to route around it. When a bug shows that a component represents something wrongly or twice, redesign that component so the correct behavior follows from a simpler, more intelligent structure, even if that means rewriting it. Prefer deleting a representation to adding one.
...
- Own halt, foreign halt and starvation differ. A halted run stops; a run whose next receive has no sent value and no install is blocked, and stays blocked at the end of its phase. A run the attacker never starts does nothing: sessions are up to k, not exactly k. A failed decryption is not a value suitable for equivalence comparison.
...
| --- | --- |
| Known is not forgeable | `aead_replay_not_forgery.vp`: `a0c0`@1, `a1c0`@2; `examples/transport-layer/piknik.vp`: `c0a0a0a0f0`@1, `c0a1a1a1f0`@2, with **replays**, never replacements |
...
| Disclosure must precede every affected receive | `causal_late_leak.vp`, `causal_foreign_receive.vp`: `a0`@1, `a1`@2; `causal_foreign_receive_early.vp`: `a1`; `causal_late_leak_forged_trigger.vp` (unguarded trigger): `a1` at both counts |
...
| Acceptance needs a matching emission, whatever the value | `auth_mac_reflection.vp`: `a0`@1, `a1`@2 (`_bound`: `a0`); `woolam_pi_pristine_pin.vp`: `a1a1`; repaired `woolam_pif_pristine_pin.vp`: `a0a0`@1, `a1a0`@2; `solver_reflected_request.vp`: `a1`; `threshold_sign_public_aggregation.vp`: `c0c0a1` |
| Query scope and held slots matter | `equivalence_halt_scope.vp`: `e0`; `precondition_halt_before_send.vp`: `a0a0`@1, `a1a0`@2 |
| A phase is judged as it stood at its barrier, through eligible holders | `phase_claims_after_compromise.vp`: `f0e0`; `phase_claims_before_compromise.vp`, `phase_claims_without_compromise.vp`: `f1e1` |
...
| Duplicate acceptance counts emissions, not honest values | `auth_withheld_static_replay.vp`, `_unchecked`: `a0`@1, `a1`@2 (the second through an unstarted sender); `_tag_guarded`: `a0`; diverted sender `auth_tampered_static_replay.vp`, `_guarded`: `a0`@1, `a1`@2; `auth_static_tag_replay.vp`: `a0`@1, `a1`@2 (`_challenge_bound`: `a0`) |
| Declaration order is not a verdict | `order_relay_declared_first.vp`, `order_relay_declared_later.vp`: `a1` at both counts |
| A relay carries its origin's emission; forwarding a genuine replay is not forging | `relay_origin_authenticated.vp`: `a0`; `relay_origin_other_sender.vp`: `a1`; `relay_scenario_genuine_replay.vp`: `a0a0`@1, `a1a0`@2 |
...
| A copy corrupt from phase N answers for its state at the end of phase N−1 | `scenario_forward_secrecy_with_honest_peer.vp`: `c1`; `_without_leak`: `c0`; `phase_claims_unrelated_leak.vp`: `f0e0` |
| A forged term reveals nothing it was forged over | `threshold_sign_forged_partial_keeps_share.vp`: `c0c1`; `threshold_sign_forgeable_is_not_a_reuse.vp`: `c1c0` |
| A phase claim leaves out later steps the run reaches, not steps it never reaches | `phase_claim_ignores_later_use.vp`, `precondition_later_phase_scenario.vp`: `f1`; `phase_claim_never_reached_use.vp`: `a0` |
...
`verify` accepts several models. `--result-code` suppresses only the banner; read the **last line** (`| tail -1`). It cannot accompany `--format json|html|tex`. HTML is self-contained; TeX compiles with tectonic. `--fail-on-attack` returns nonzero for attacks. `--auto-queries` replaces queries. Other commands include `about`, `diagram` (Mermaid), `lsp`, `completion` and hidden `man`. JSON/HTML/TeX/LSP share `src/report.rs`; editor plugins are thin clients.
...
- Thread-local caches use `context::Generational`, clearing on first access in a different generation to prevent cross-model id contamination. **Internal parallelism only with one live analysis**: `live_generations() > 1` makes `map_ordered` sequential; only `GenerationGuard` registers a live generation. `DeductionMemo::scoped` installs/restores the theory's deduction memo.
...
parse_file (parser.rs)              hand-written recursive-descent, comment-preserving AST
  ...
  → sanity (sanity.rs)              model validation; drives construct.rs
      → construct_protocol_trace    "km": ProtocolTrace — the one protocol description: slots
                                    with their delivery facts, and the CapabilityIndex
...
`verify::analyze(&Model)` is the **one** place that sequence exists (its scenario and session expansion is `verify::expand`: `expand_scenarios` and `expand_sessions` fill one `verify::Expansion`, which the LSP's live check also runs, so its diagnostics agree with the analysis); it is `analyze_sessions(m, DEFAULT_SESSIONS)`, and `wasm_verify` and the LSP call it too, while `wasm_analyze` takes the CLI's `verify_parsed` path at `DEFAULT_SESSIONS` unless asked otherwise — **do not give one entry point its own default.** Throughout, `km` is the `ProtocolTrace`, `cx` the engine `Context` (program, `km`, initial knowledge) and `ex` an `Execution`.
...
`execute(cx, installs)` with `Installs = Vec<(run, slot, value)>` produces one `Execution`: per-run environments of `Held { value, pre, sender, installed, authored }`, program counters, halts, recorded sends (`sent[d]`), the order in which steps ran, one `Knowledge`, and, when the model has phases, the configuration archived at each barrier before the last (`barriers`; `Execution::at(p)` returns the configuration at the end of phase `p`, the execution itself for the last).

- Phases are barriers. Within phase N every run advances while its next step belongs to a phase ≤ N; the loop repeats until no run moves. A receive waits for its sender's send; only when no run can move does one pass deliver each receive still waiting on an unsent message whose installs are all derivable, as installs, and the loop resumes. Enabling is monotone within a phase (knowledge only grows, recorded sends are never retracted), so this is the unique maximal execution for the install map, independent of run order: an install equal to what its sender is about to send is never delivered ahead of that send and so never counts as attacker-authored (`order_relay_declared_first.vp`/`_later.vp`).
...
- An install at `exec::UNSTARTED` leaves its run `idle`: frozen before its first step, so it sends nothing and the runs waiting on it starve until `fill` supplies them (`auth_static_tag_replay.vp`, `wire_projection_replay.vp`).
- At a phase end, a run blocked at a receive that carries an install is `stuck` (the install never became derivable); unguarded slots without installs of an unsent delivery a run waits on are `withheld`. Runs that could not finish the phase are `frozen` and never resume. Knowledge closes at each phase end and at the end; each barrier then archives a copy of the whole configuration (run states, sends, order, knowledge).
...
`Knowledge` wraps one `AttackerState` with an `Origin` per value (`Initial`, `Wire { run, slot }`, `Leak { slot }`, `Derived(DerivationRecord)`) and the `protocol` terms (value, unreduced form, produced-by-a-principal flag) seen in the execution. `close(capabilities)` is a monotone fixed point over fresh snapshots:

1. Held values: `can_decompose`, `can_break_weak`, `rewrite_build`, and core-reveal fragments. A term held only by forgery (its record is `Broken { Forgeable }` or `ReusedForge`), or one the attacker could mint and no principal built, is never decomposed: every other argument was the attacker's, and the forged-over secret must stay hidden (`threshold_sign_forged_partial_keeps_share.vp`).
2. Reconstruction candidates (every subterm of protocol terms, plus each combine rule's whole for a walked partial: `threshold_join_unblinded.vp`): `can_reconstruct_primitive`, recording `Reconstructed`, `Combined`, `Broken` (capability) or `ReusedForge`.
...
`derivable(v, capabilities)` closes, then asks `theory::obtainable`. Every learned value keeps its `DerivationRecord`; narration depends on it. `origins` records how a value was first acquired and is what narration reads; `state.derivations` records the strongest way it is held: a later wire/leak disclosure of a value first derived replaces its record, so withholding the derivation's ingredients no longer removes it (`unlink_origin_late_leak.vp`, `c1u1`). Phase snapshots keep the record current at their phase.
...
`Judge { cx, ex, whole, claims }` evaluates one query against the configuration an execution reached at the end of one phase (`ex.at(p)`, whose knowledge is K_p): a claim at phase `p` is about what runs honest at `p` had done by then, never about later values, uses or sends (`phase_claims_after_compromise.vp`, `f0e0`; counterweights `phase_claims_before_compromise.vp`, `phase_claims_without_compromise.vp`, `f1e1`). `claims` is `VerifyContext::claims_at` (scenario corruption): the phase whose configuration a run answers for. A copy honest at `p` answers for `ex.at(p)`; a copy corrupt from phase N ≤ `p` answers for its state at the end of phase N−1, judged against K_p, which is forward secrecy (`scenario_forward_secrecy_with_honest_peer.vp`); a copy corrupt from phase 0 answers for nothing, unless every copy is, when all answer so that a fully corrupt model does not hold vacuously. `whole` is the execution the configuration belongs to. `Judge::claimed(r, slot)` is the value a claimed run answers for: its own `Held`, else its creator run's when that creator is claimed, so an eligible run never answers for a corrupt-peer run's values; a slot no run creates (`nil`) is its declared value. `evaluate` first requires every precondition's named send to have executed in `whole` (a precondition restricts which executions a query is about, so a send in a later phase still admits a phase-`p` claim: `precondition_later_phase_scenario.vp`), then returns a `Violation` and a `Verdict` token.
...
- **Authentication:** injective agreement at the recipient run `b`. The slot must be held with a sender and used positionally (`uses`: every slot that mentions it reached, one use succeeding or unchecked, where a use fails exactly when its run halted at it, which holds because every checkable primitive fails only through its declared rule (`a_checkable_primitive_fails_only_through_its_declared_rule`); at a phase barrier a use the run reaches only in a later phase is left out, while one it never reaches still cancels, or removing a phase boundary could lose an attack: `phase_claim_ignores_later_use.vp`, `phase_claim_never_reached_use.vp`). A slot mentions it through the run's own assignments. Only when the run has no such slot are its uses found as sanity finds them, through other principals' computations (`mentions_across_principals`): `auth_use_through_guarded_relay.vp` (`a1`), `auth_use_through_checked_relay.vp` (`a0`). Widening this for runs that do use the value directly adds later unreached uses and loses Lowe's attack (`spore_ns_pk.vp` at one session). An unauthored value received straight from the named sender is matched by that send. Otherwise count emissions: sends of an equivalent value at sibling slots by runs interchangeable with the named sender (`km.interchangeable_for`, per slot) on deliveries that reach the recipient's agent (`km.same_actor`) directly or through principals relaying that slot, counted only when some sender's copy of the value is not authored, since a relay forwarding a genuine replay is not forging (`relay_scenario_genuine_replay.vp`). An unauthored value from another principal with no emission: `Substituted` (`relay_origin_authenticated.vp` against `relay_origin_other_sender.vp`). An authored value with no emission: `Forged`, whatever the value: an attacker-delivered copy of the honest value that the sender never sent is not agreement (`auth_mac_reflection.vp`, `threshold_sign_public_aggregation.vp`). Otherwise count acceptances of that value across the agent's runs; more acceptances than emissions: `Replayed`, reported as `DuplicateAcceptance` when the recipient contributed a fresh value to it and `ReplayableFirstFlight` otherwise.
- **Freshness:** a claimed run holds and uses a value with no fresh leaf, or an authored value another run of its agent also accepts at a sibling slot (`repeated`: the attacker kept it the same across sessions): `Stale` (`freshness_replayed_across_sessions.vp`). Freshness generates no solver proposals; the Constructed pass's sibling flights supply the replays.
- **Equivalence** compares, per claimed run, the `claimed` value of every queried slot; borrowing from creators is what cross-principal equivalence needs (`k_alice`, `k_bob`). Every queried slot must be claimed in this configuration (never the honest fallback of an assignment that did not run: `equivalence_unexecuted_assignment.vp`) and not a failed check; a claimed value was computed, so no reachability or starvation gate applies (`equivalence_borrowed_while_starved.vp`, `equivalence_knows_creator_halts.vp`, `equivalence_halt_borrowed_order.vp`). Carrier slots are not restricted: corrupt-peer runs still disclose.
- **Unlinkability** requires both slots of a pair claimable and asks `engine/unlink.rs::link` about run `r` of the configuration directly: no projected state.

`link` receives the claimed pair, rejects `authored` values, and requires both observable: known, and extracted from what this configuration disclosed (`ex.sent` and executed leaks) by tuple projection, decomposition, weakening or a held reuse pair, or assembled as a tuple of such values, or a public constant. Halts and phases need no separate treatment: the execution only records what happened. It then collects each value's **ties**, the secret-dependent terms the attacker can connect to it: the value itself (`Itself`), the ingredients it can rebuild the value from without it (`Built`), the input a runnable check verifies it under (`Key`) and the arguments a runnable check confirms, with their ingredients (`Confirms`). Witnesses the attacker installed anywhere in the execution are rejected (`unlink_supplied_witness_only.vp`, counterweight `unlink_supplied_witness_masks.vp`). A shared term whose pair of ties is in `strongest`'s table is a link, a check's key or confirmed argument also recognizing the other value itself or an ingredient of it (`unlink_signature_and_its_key.vp`), ranked equality, identifying check, common origin, own value as an ingredient of the other, recognized secret; failing those, the ties of tuple components and rebuild ingredients (identifying check and recognized secret only). Every candidate passes every gate before it is ranked. These predicates are not two-world behavioral equivalence. Preserve all gates:
...
- A proposal becomes installs at the proposing run when it receives the slot directly (one receiver when addressed), else at the unguarded receivers upstream of it through guarded relays, bounded by `TermBound::admits_at` (a cut notes `Truncation::TermDepth`). `bases` adds the installs of accepted nodes whose executions derive values the proposal needs; a value not yet derivable is allowed only when the closed union plus what the proposal makes principals emit (`emissions_under`: any wire or leaked term mentioning a variable) derives it: an oracle chain, delivered because a victim run discloses it first, possibly post-processed (`oracle_output_hashed.vp`). Unless addressed, a `shared` probe also installs the value at every other unguarded receiver that uses or relays the slot, since the symbolic state solved the proposal as if they all received it (`broadcast_consistent_replacement.vp`). Probes are executed and judged but never absorbed or kept: absorbing them reordered which executions teach the union, and `unguard` then lost `solver_oracle_input_certifies_key.vp`'s oracle attack.
- Candidate families: `base`; `fill` (the `withheld` slots of an unsent message a run waits on take the honest value if derivable, else nil: a replay into a receive whose sender halted, or a trigger the attacker supplies itself, `causal_late_leak_forged_trigger.vp`; and, at a run halted here but not in the honest execution, each unguarded received slot holding something other than its honest value takes the honest value when derivable: an upstream install diverted the sender, and the attacker restores the input from another session, `auth_tampered_static_replay.vp`); `repair` (`Deducer::repair_check` rebuilds a halted check's inputs, retried while the halt moves later); `drop` (every single-install removal of a multi-install single-run flight); `merge` (sibling-copy transfer of accepted installs; merges of the nodes supplying a confidentiality target's derivation leaves); `stuck` (a stuck candidate retried with each accepted node that freshly emits the stuck value, never cascading; sources are kept per value, emitting run and set of runs the source installs at, since sources touching different runs are different ways to produce the value); `cleared` (when that merge stays stuck, the same merge without the candidate's installs inside the causal cone of the source's emission that disagree with what the source's execution received there: solver fillers such as `nil` otherwise undo the source, `woolam_pi_pristine_pin.vp`); `admitted` (a stuck candidate's non-stuck installs); `idle` (after the fixpoints, each accepted node with one more run left unstarted: a run interchangeable with an unresolved authentication query's sender whose emission some other run accepted in that node's execution). `fill` applies to every executed candidate, its own included: fills only add installs, so the chain ends (`receipt_chain_broken_link.vp`). A `stuck` retry also takes the source node's installs inside the causal cone of its emission, even at runs the candidate already touches (`search_chain_through_server.vp`).
...
- `deduce.rs` tries held values, variables, replay, wire unification (two-way: a principal's own symbolic emission against a goal whose fields are still free, `solver_reflected_request.vp`), finite-basis oracles, rewrite matching, argument construction, decomposition and check inversion. Memo keys include the **whole substitution**; never cache cycle-cut results. Rewrite inversion carries every unifier; nested decomposition retains every outer opening requirement. `repair_check` serves the search's repair family.
- `diverge.rs` distinguishes installed tuple projections using derivable fillers; `solve/mod.rs`'s `keyed_free`, `preserved_free`, `aligned_held_free` restrict fillers to protocol terms. Arbitrary held terms inflate the basis.
- `mod.rs::propose`: query goals, constraint goals, oracle input goals (another principal's check shapes, including its equality checks inverted into shapes for its own slots, unified with this principal's emissions: `solver_type_flaw_nested_pair.vp`), blanket substitutions, single-slot substitutions, then Constructed sibling flights and protocol/rewrite candidates. Proposal lanes run on `map_ordered` with disjoint free-variable bands. `leave_honest_slots` removes honest bindings unused elsewhere; `install_signature` dedups by reduced concrete installs. Do not revive `skeleton.rs` or give the attacker shapes it did not derive.

### Parallel sessions (sessions.rs)

`expand_sessions` clones principal/message blocks before sanity. Scenario and session expansion share `sessions::ModelCopy` (principal id/name allocation, `c#s`/`c@k` renaming, `copy_value_id` offsets). Fresh/assigned constants become `c#s`; `knows` constants stay shared. Scenario/session expansion requires `s*k <= 31` (`MAX_COPIES = 30`). Original queries cover session 1; variants share `query_index`. Keep scenarios and sessions distinct. `km.session_siblings` and `km.copy_siblings` group a constant's copies; `km.interchangeable` and `km.actors` group principal copies for sender matching and recipient agents. Clones are runs, not agents: match recipients by agent, compare scenario interchangeability per slot (`interchangeable_for`); base-name canonicalization excuses Lowe's attack.
...
- `compromised_constants` asks what is computable. Seed secrets leaked/sent bare at disclosure time, including assigned private keys and tuple-exposed secrets. Generated constants qualify only at registry `secret_positions`; public identifiers/salts do not. Follow aliases/nested tuples through `reveals_args`, and a decomposable term through its declared reveals once every `given` input (read through the rule's filter, a bound constant resolved to its assignment) is computable, dated at the later of disclosure and key (`scenario_corrupt_wrapped_key.vp`); opaque arguments stay secret. Derivations need **every** ingredient public, nil, compromised or observed (any value disclosed on a wire or leaked, secret or not: `scenario_corrupt_by_observation.vp`); date them at the latest ingredient.
- `scenario_corrupt_from` asks whether a binding identifies a controlled peer: a bound value may be compromised **or mention** one. Key material is a constant at a registry secret position, the outputs of an assignment with a computed term there (`scenario_corrupt_inline_key.vp`), and a bound value whose target is key material (`scenario_corrupt_bound_key.vp`). Query variants cover scenarios honest at phase 0, or every scenario when none is, so declaration order never decides which is asked (`scenario_all_corrupt_order.vp`). `ScenarioSummary::corrupt_from` labels a scenario turning corrupt later as a peer corrupt from that phase.
- `VerifyContext::is_honest_at` governs honest-run checking (`check_honest_run` reads the root execution: its least halted slot whose creator is honest there is the model error, then argument restrictions over every held value; this equals evaluating every slot statically only because a run is blocked solely behind an earlier halt in its own copy and corruption covers a whole scenario copy from one phase, so keep both true or check unreached slots too) by slot creator at the phase of the failing assignment's program step (honest failed checks are model errors; corrupt creators just halt: `scenario_corrupt_at_late_check.vp`, counterweight `scenario_corrupt_after_check.vp`); `claims_at` governs query eligibility.

Acceptance pair: `spore_ns_pk.vp` (`c1a1a0`, Lowe attack), `spore_nsl_pk.vp` (`c0a1a0`). **Do not add scenario-free variants to `examples/`.**

### Verdict envelope

`VerifyResult` carries `Envelope { sessions, truncations }`, printed after PASS (`[search exhausted at 2 sessions]` / `[search truncated: term depth]`) and serialized as `QueryReport.envelope`. Attacks are unqualified by envelopes; FAIL may carry `Subtype::{AttackerSuppliedValue, DuplicateAcceptance, ReplayableFirstFlight}`. `VerifyResult.notes` supplies `Note:` lines; spanless auto-queries set `QueryReport.generated`. Empty notes/generated fields are omitted from JSON. Declared weakening assumptions are the annotated applications as the source model writes them (`capability::declared_assumptions`, before scenario/session expansion), stored on `VerifyContext`. Equivalent applications merge; each merged capability becomes one row whose term carries exactly that capability at its earliest onset, with nested annotations removed since they have their own rows. Every format therefore agrees with the row's fields regardless of declaration order (GitHub issue #44). Never render the `CapabilityIndex`, whose resolved entries expand shared term graphs (`cap_weak_shared_chain_assumption.vp`). `Truncation::TermDepth` and `Truncation::SolverVariables` attach to queries **unresolved when first encountered**; `finalize_envelopes` preserves that scope.
...
There is no saturation mode (`--saturate` was removed, GitHub issue #43). Agreement between adjacent session counts is no evidence that a higher count finds nothing: a 4-of-4 threshold secret needs four concurrent runs to recover and holds at one through three. The session count is chosen explicitly with `--sessions`.
...
- **Capabilities do not affect term identity.** Read the trace's `CapabilityIndex` (`km.capabilities`, which the theory takes), never a deduped held term. Collect annotated calls independently; reduce arguments while retaining annotations.
- There is no per-principal state. Sanity and the solver read `km` plus a `PrincipalId`: per-slot facts live on `TraceSlot` (`mutatable_to`, `delivery_phase`, computed once from `sent_by`), per-principal facts are derived from it (`sender_to`, `guarded_for`, `sent_from`, `mutation_reaches`), and honest values are `initial_value`. Runtime state lives only in the execution (`Held` keeps `value` and `pre`).
...
- `can_reconstruct_primitive` refuses irreducible core applications: `ASSERT` is a check; reducing `SPLIT` still reconstructs. A failing unchecked non-core application is a value like any other and is reconstructed from obtainable arguments (`unchecked_signature_failure.vp`); a failing checked one is not, since its run halts there.
- `obtainable` opens a `DeductionMemo` scope when absent for its capability index/snapshot, shares subproofs within direct calls and restores enclosing scopes (`shared_transcript_bypass.vp`, `c0`).
- **No recursion cap.** `to` rules return subterms or reassemble strict subterms: depth descent is well-founded.
- `can_rewrite` is the sole reducer: argument reduction, `can_rebuild`, `can_combine`, then rewrite. Report failed outer checks, preserve partial argument reductions (`failed_checks_report_the_term_with_its_arguments_reduced`).
...
- `parser.rs` preserves comments, always builds the token index and skips a leading BOM by position. After a primitive's `)` or `?` it stays on the line, so a comment on the next line is never taken as trailing. `pretty.rs` is pure, idempotent, golden-tested and **not sanity-gated**; it owns AST `Display`. `resolution.rs` follows bare aliases with cycle guards; registry argument normalization bounds forbidden nesting.
- `info.rs` handles output and silent probes. `update.rs` makes the sole outbound request (GitHub tags), **only when stdout is a terminal**; network work under pipes delayed exit.
- `lsp/server.rs` uses stdio, debouncing, worker analysis (`analysis.rs`) and documents kept in a map keyed by URI, each a `state.rs` `Document` (its URI, line index and text, parse, tokens, trace and first parse or validation error), without filesystem access. Only the server, `analysis.rs` and `proto.rs` need `lsp`; the rest also serves the WASM language exports. **Both threads set Silent verbosity** or thread-local output corrupts stdout. Keyword/query prose lives in `docs.rs`; primitive/capability docs come from specs.
- `report.rs` supplies JSON/HTML/TeX/LSP. Ranges use byte offsets and 1-based line/column, not LSP positions. Parse once in `Run::of`; renderers never parse. Renderers share its derived facts (`Run::tally`, `Analysis::scope`, `Analysis::attacked_values`) and `msc::Chart`, which alone decides whether a diagram has anything to draw. Include leak-only principals. Session/scenario suffix notes appear only when a structured trace step names a `#`/`@` value.
- **No HTML/LaTeX markup in Rust.** Embed templates/assets; `Val::Text` escapes, `Val::Raw` does not. Tests reject missing/unused placeholders. LaTeX figures retain paired extraction markers and `\vp…` macros; `listing_safe` excludes embedded `\end{lstlisting}` and maps box-drawing glyphs, which the listing font lacks, to ASCII. The preamble lays out and paginates charts itself, so Rust estimates no heights: every row and every box line is a strip measured where it is drawn, spanning the chart's fixed width so all strips scale alike; the column is set whole when it fits (or moves whole when under a page and less than half a page remains), otherwise `\vsplit` at each page end under repeated plates, and always whole inside a float or minipage (`\ifinner`). Verdict and summary tables are `longtable`s with fixed column widths. Keep `\color` out of titlesec's vertical-mode format argument: its whatsit lets a page break separate consecutive headings. Preserve the no-proof disclaimer. Bless goldens with `VERIFPAL_BLESS_HTML`/`VERIFPAL_BLESS_TEX`; `VERIFPAL_TECTONIC=1` enables compilation.
- `msc.rs` shares diagram rows; every value of a `mutations` step is an install, even one equal to the honest value. `diagram` declares participants by id (`participant pN as Name`), so principals named like Mermaid keywords stay valid. `tokens.rs` indexes even failed parses. **Do not add `Span` to `Constant` or process-global mutable state.** Interners are parser-owned and error before ids enter attacker/copy/solver bands.
...
Known sensitive sites include the key-shaped slot variables (`symbolic::shaped_var` offers a slot whose honest value is a public key only `PUBKEY(var)`, which every Diffie-Hellman man-in-the-middle needs, so a non-key value standing in for a key is never proposed: the one-sided domain tag in `solver_oracle_input_domain_separated.vp`'s history), Targeted-before-Constructed order, finite protocol-term candidates, aligned-position filling, substitution-sensitive goal memos, which executions the search keeps, failed-check/truncated-use acceptance and failed-decryption equivalence. Authentication's positional acceptance is specified, not a bookkeeping bug: a successful use before an unrelated failed check may count; a later unreached use of that constant cancels it. Changing this requires a semantics decision.
