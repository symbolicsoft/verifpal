...
| Duplicate acceptance counts emissions, not honest values | `auth_withheld_static_replay.vp`: `a0`@1, `a1`@2; `_unchecked`, `_tag_guarded`: `a0`; diverted sender `auth_tampered_static_replay.vp`: `a0`@1, `a1`@2; `_guarded`: `a0` |
| Declaration order is not a verdict | `order_relay_declared_first.vp`, `order_relay_declared_later.vp`: `a0`@1, `a1`@2 |
...
- `VerifyContext::is_honest_at` governs honest-run checking (`check_honest_run` reads the root execution: its least halted slot whose creator is honest there is the model error, then argument restrictions over every held value; this equals evaluating every slot statically only because a run is blocked solely behind an earlier halt in its own copy and corruption covers a whole scenario copy from one phase, so keep both true or check unreached slots too) by slot creator at the phase of the failing assignment's program step (honest failed checks are model errors; corrupt creators just halt: `scenario_corrupt_at_late_check.vp`, counterweight `scenario_corrupt_after_check.vp`); `claims_apply_at` governs query eligibility.
