...
cargo test --release                   # 439 tests (unit + model), ~2s once built (also: make test)
...
Nearly every model in `examples/test/` runs in milliseconds. The five `junglegym_*.vp` stress models are the exception — deliberately convoluted, and the slowest four in the tree: `junglegym_deep_ratchet.vp` ~1.0s, `junglegym_threshold_ring.vp` ~0.44s, `junglegym_hybrid_pq.vp` ~0.28s, `junglegym_password_maze.vp` ~0.18s. They exist to lean on resolution, the theory descent, `normalise_arguments`, guard bypass and the unlinkability witness search all at once; each carries its expected result code and the reasoning for it in a header comment. `cargo test --release` as a whole is ~2s once built, so `junglegym_deep_ratchet.vp` is a noticeable slice of it but not most of it.

Of the shipped real-world models, `examples/transport-layer/tls13.vp` is the slowest that completes (~0.9s), then `signal.vp` (~0.55s), `needham-schroeder.vp` (~0.5s), `piknik.vp` (~0.45s), `userbase.vp` (~0.25s) and `scuttlebutt.vp` (~0.12s); `firefox-sync.vp`, `cen.vp` and `lc-dp-3t.vp` are all under 10ms. `needham-schroeder.vp` used to run for many minutes without resolving a query; the self-feeding replay cut in `solve/validate.rs` is what brought it down. The slowest model in the tree is `examples/messaging/signal_twelve.vp` at roughly 50s, which is 24 queries over twelve ratchet steps and is expected to be slow — it is also the only model large enough for `solve/vars.rs::dedupe` to matter, so it is the one to measure after touching the solver, and the one not to put in a timing loop.

Use `--release` for anything that actually runs analysis: the debug build is roughly 14× slower (`tls13.vp`: 0.9s release, 12s debug).
