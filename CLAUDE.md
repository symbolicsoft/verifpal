...
| Duplicate acceptance counts emissions, not honest values | `auth_withheld_static_replay.vp`: `a0`@1, `a1`@2; `_unchecked`, `_tag_guarded`: `a0` |
...
- **Equivalence and unlinkability** reuse the `PrincipalState` evaluators: `view::project` turns run `r` of the execution into a state (values, `pre_rewrite`, `original` from the honest run where authored, provenance, `foreign_halts`, `starved`, `halted_at` truncation). A view borrows the slots its run does not hold from their creators, which cross-principal equivalence needs (`k_alice`, `k_bob`); `view::claimable` admits a queried slot only when the run holds it or its creator is also eligible at `p`, so an eligible run never answers for a corrupt-peer run's values. Equivalence requires every queried slot claimable in this configuration (never the honest fallback of an assignment that did not run: `equivalence_unexecuted_assignment.vp`), reached, not starved and not a failed check. Unlinkability requires both slots of a pair claimable and calls `unlink::find_link_witness` over the projected state and the phase knowledge. Carrier slots are not restricted: corrupt-peer runs still disclose.
