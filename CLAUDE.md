...
- **Equivalence** reuses the `PrincipalState` evaluators: `view::project` turns run `r` of the execution into a state (values, `pre_rewrite`, `original` from the honest run where authored, provenance, `foreign_halts`, `starved`, `halted_at`). A halted run's view is not cut at its halt: its unreached own slots are neither held nor claimable, and cutting by slot index also dropped borrowed slots declared later in the file (`equivalence_halt_borrowed_order.vp`). A view borrows the slots its run does not hold from their creators, which cross-principal equivalence needs (`k_alice`, `k_bob`); `view::claimable` admits a queried slot only when the run holds it or its creator is also eligible at `p`, so an eligible run never answers for a corrupt-peer run's values. Equivalence requires every queried slot claimable in this configuration (never the honest fallback of an assignment that did not run: `equivalence_unexecuted_assignment.vp`), reached, not starved and not a failed check. Carrier slots are not restricted: corrupt-peer runs still disclose.
...
- `PrincipalState` remains the input shape for sanity, the solver and equivalence: `original` = honest computation/belief; `pre_rewrite` = mutated/unreduced; `value` = mutated/reduced. The engine's own `Held` keeps `value` and `pre`; `view::project` fills the rest.
