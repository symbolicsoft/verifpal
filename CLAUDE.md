...
| `bypass_keeps_other_checks_honest.vp` | `a0a1`@1, `a1a1`@2 | a bypass round re-resolves the *pre-resolution* state once with every bypass applied; resolving a resolved state again loses inlined provenance and verifies `SIGN(sk, x)` against a substituted `x` |
...
- Each bypass round rebuilds from the pre-resolution state with every bypass so far and resolves **once**. `resolve_all_values` is not idempotent under installs: a resolved state re-resolved reads the constants it inlined from other principals through the reader's own perception (`bypass_keeps_other_checks_honest.vp`).
