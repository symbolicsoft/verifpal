...
**Declare primitives only in `src/primitive/spec.rs`.** `PrimitiveSpec` / `PrimitiveCoreSpec` own names, arities, outputs, argument names/restrictions/errors, decomposition/recomposition/rewrite/rebuild/reuse, checking keys, commutativity, key/identifying roles, capabilities/reveals, tuple/projection/equality/divergence roles, help and editor docs. The engine interprets fields.
...
| `closure_cyclic_union.vp` | `c1c1c0`@1 | two installs whose union defines each slot in terms of the other: a replay that cannot execute is refused, not admitted because no state came back. At two sessions a second Bob supplies the other tag (`c1c1c1`) |
...
| `causal_late_leak.vp` | `a0`@1, `a1`@2 | a key disclosed only downstream of the check it would forge is not held in time — an admitted forgery cannot spend it. `signal_small_leaks.vp` is `c0a1`@1 for the same reason |
...
| `bypass_needs_the_signed_message_public.vp` | `a0` | a deterministic signature over a fixed message supplies no different accepted term. Counterweight `bypass_needs_the_signed_message_wire.vp` (`a1`) |
...
| `phase_relay_guarded_delivery.vp` | `a1a0`@2 | the open first leg admits replay, while guarded forwarding of the honest sibling value adds no duplicate |
...
| `scenario_corrupt_by_leaked_wrapped_key.vp`, `scenario_corrupt_by_wrapped_key_on_wire.vp` | `c0` | a key exposed by a leaked or sent tuple is compromised at the tuple's disclosure phase, including nested tuple aliases. Counterweight `scenario_corrupt_by_opaque_wrapped_key.vp` (`c1`) exposes only a hash and keeps the peer's claim |
| `unlink_shares_recomposed_secret.vp` | `c1u0` | a secret assembled out of the two queried values is not their common origin; a witness must stay derivable with either value and everything derived through it withheld |
| `halted_peer_relay_holds.vp`, `halted_peer_precomputed_relay_holds.vp`, `halted_peer_downstream_reencrypt_holds.vp` | `c0` | a halted peer's message was never delivered, so nothing downstream of it was computed, in *every* principal's view; phase seeding and the closure must not publish or resolve a name through a slot the honest run never reached. Counterweights `halted_peer_relay.vp`, `halted_peer_downstream_reencrypt.vp` (`c1`) |
| `bypass_keeps_other_checks_honest.vp` | `a0a1`@1, `a1a1`@2 | a forged ciphertext does not change the honest signature over the separately received message |
| `causal_foreign_receive.vp` | `a0`@1, `a1`@2 | a value is judged against the earliest delivery to **every** principal the install reaches, not only the re-executed one: reflecting Bob's later nonce into a slot delivered to a Server is a causal cycle. Counterweight `causal_foreign_receive_early.vp` (`a1`) leaks that nonce first. The same false attack appeared in Kao-Chow |
| `causal_alt_route_late_leak.vp` | `a0`@1, `a1`@2 | an alternate state read is consulted only for a value its primary route leaves unreachable, and only where the reading execution was handed nothing but public constructions: an oracle output whose recorded route runs through a key leaked after the attacked check is not held in time. Counterweight `causal_alt_route_early_leak.vp` (`a1`) |
...
| `matching_run_unreached_send.vp`, `matching_run_unreached_send_checked_first.vp` | `a1` at both counts | a tag computed before a failed check cannot excuse acceptance when the sender never reaches its send; moving the check before the computation preserves the attack |
| `matching_run_recipient_halt.vp` | `a0a1`@1, `a1a1`@2 | a send reached before a later halt can match that recipient's acceptance, but not another recipient's unreached send; the emission cache includes the recipient |
...
| `halted_peer_relay.vp`, `halted_peer_downstream_reencrypt.vp` | `c1` | a send the honest run never reached is forwarded from the execution that reaches it (`forwarded_installs` via `honest_run_delivered`), the recipient adopts the executed sources' actual halts (`adopt_foreign_halts`), and what it then sends is republished (`attacker_absorb_disclosed`) |
| `matching_run_leaked_derived_key.vp`, `matching_run_public_key_output.vp` | `a1` | `forgeable_without_sender` strips only `run_dependent` reads: a value computable from the run's own static and fresh data is disclosed in every execution, so a leaked derived key or a public key is the attacker's own capability. The old rule violated `unguard` (guarding `n` at Alice gave `a1`, unguarding it `a0`) |
| `scenario_generated_identifier.vp`, `scenario_generated_salt_mentioned.vp` | `c1` | a generated value is a compromise seed only where it is used as a secret (`primitive::secret_positions`); an identifier or salt sent in the clear does not make the peer corrupt, and the run keeps its query instance |
| `solver_oracle_input_prefix.vp` | `a1` | a refused joint proposal falls back to its `admitted_prefix` (installs already controllable, derivable and causally available), so an oracle input whose shape only a later check dictates is proposed alone and the next round completes the chain. `…_private_tag.vp` (`a0`@1) withholds the shape |
| `solver_oracle_input_other_principal.vp`, `solver_oracle_input_salt.vp`, `solver_oracle_input_certifies_key.vp` | `a1`, `a1`, `c1a1` | an oracle's input must take the form another check demands: a check at a third principal, a salt the goal solver's one-way matching could not bind, and a constant-shaped name slot filled with an attacker key |
| `split_delivery_equivalence.vp`, `split_delivery_equivalence_checked.vp`, `split_delivery_forgery.vp`, `split_delivery_rewrap.vp` | `e1`, `e1`, `a1`, `c1c1` | one constant sent unguarded to two recipients can be replaced for one of them only: an **addressed** install (`SlotValues::addressed`) reaches no other principal's view, and `solve_principal` runs the Targeted pass once more over `symbolic::build_addressed` for a principal with such a slot. `split_delivery_equivalence_guarded.vp` (`e0`@1) has nothing to address |
| `closure_route_collision.vp`, `closure_route_collision_sessions.vp` | `c1` | when two ingredients' primary routes need different values at one slot, `gather_reads` tries each read's recorded alternate routes before refusing; only routes through a slot that travels or leaks count, because equivalize records a route at every slot holding a known value |
| `solver_oracle_chained_inputs.vp` | `a1` | the shapes a check demands include the ones nested inside them, so two oracles can be walked in series: proposing only the outer tag asks each oracle for a term nothing derives yet. Filtered by emission head; counterweight `solver_oracle_chained_inputs_separated.vp` (`a0`) separates the domains |
| `split_delivery_relaying_recipient.vp` | `e1` | an addressed install is refused only where the *other* recipient is served through the addressed principal: a principal that forwards the slot on to somebody else can still be handed a different value itself |
| `starved_by_a_later_declarer.vp` | `e1` | a halt withholds what a principal computes, not what it already knows, so `slot_held` credits a principal's own `knows` declaration before asking whether the slot's first declarer halted |
| `precondition_independent_late_block.vp` | `a1a1` | a precondition names a message in the protocol, so its constant resolves against the trace; resolving it against the answering state dropped the verdict whenever that state was truncated before the constant was declared |
| `precondition_upstream_answerer.vp` | `c1c0` | a gate naming a message whose sender halts is judged in the disclosing execution, not from whichever state happened to hold the secret |
...
`solver_constructible_reduct.vp` (`c1` at both counts): encrypting nil twice to Bob makes his decryptions reduce to nil and his wrapping key public. Require the **reduct** derivable, not the unreduced decryption's private key. `solver_constructible_reduct_guarded.vp` (`c0`) guards input 1.
...
cargo test --release                   # unit + model tests (also: make test)
...
**Threshold:** required only where spec declares `threshold`, e.g. `THRESHOLD_SPLIT[3](k)` or `[3, weak]`. Errors: `err_threshold_missing.vp`, `err_threshold_on_hash.vp`, `err_threshold_twice.vp`, `err_threshold_one.vp`, `err_threshold_above_shares.vp`. `Primitive.threshold` participates in `primitive_hash`, `equivalent_primitives`, `structurally_identical_primitive`; shares at different thresholds never combine (`threshold_thresholds_are_distinct.vp`). A spec declaring `distinct_per_assignment` (`THRESHOLD_SPLIT`) also stamps `Primitive.instance` from the assignment's first output constant, so two sharings of one secret are two term families and their shares never interpolate together (`threshold_reshare_is_a_new_sharing.vp`); the stamp is the base constant id, shared across session and scenario copies. Output count n belongs to the assignment, not term; `MAX_SHARES = 16`.
...
- **Authentication checks duplicate acceptance.** A sibling replay is reported only when joint re-execution establishes successful uses by both recipient clones of the same sibling value, with compatible worlds and a reached source send. Accepting a sibling session's honest wire value twice is an attack without a forgery. `session_sibling_replay` narrates it as "replays". `copy_siblings` spans both expansion axes. Joint duplicate-acceptance checks run before `emitted_by_matching_run`; narration then labels the established cross-scenario replay (`scenario_replay_is_a_replay.vp`). `sibling_flight_substitutions` remains session-only.
...
- `forgeable_without_sender` transitively removes reads of **run-dependent assignment outputs** from sender runs (`run_dependent`: some leaf of the resolved definition is neither `knows` nor created by the run); a still-`derivable` value is a forgery (`matching_run_forged_unguarded.vp`). Keep generated keys (`matching_run_leaked_generated_key.vp`) and derived or public values the run emits in every execution (`matching_run_leaked_derived_key.vp`, `matching_run_public_key_output.vp`). Then try three sender re-executions: **mirror** (`delivered_to` this recipient), **record** (tainted diffs where `Obtained` at the sender's own slot), **routing** (sibling flight).
...
`attacker_phase_update` publishes reached wire values before principals run, allowing concurrent routing independent of block order. That knowledge still must be available **before the attacked receive/check** (`atemporal_forward_value.vp`). `available_before_receive` finds the earliest delivery to **each** principal the install reaches, the re-executed one and, unless the install is addressed, every recipient in `mutatable_to`; `held_before` restricts knowledge at that set and keys its caches by it; `close_influences` computes blocked trace slots as a fixed point over sends. Filtering by the re-executed principal's own receives alone skips the check entirely for a slot delivered to a third principal (`causal_foreign_receive.vp`). Receives and guard checks share this filter.

- Block by **disclosure**, not declaration (`causal_late_leak.vp`); except `nil` and public constants.
- Concrete forgeries are admitted at the delivery they replace. Their ingredients must be causally available there (`noise_xx_mutual.vp`).
- `restrict_known` follows primary derivations through `reachable_knowledge`, retaining unblocked reads and ingredient-free constructions, never comparing pristine trace values (`later_value_fed_back.vp`). A value whose primary route is blocked is still retained through an alternate state read at an unblocked, observable slot whose record's installs are all `public_construction`s (built from `Initial` knowledge alone): such a read holds in every execution, which is what lets an oracle output recorded as *built from* a key leaked only after the attacked check be spent as the read it also was. Any wider use of alternates here returns `atemporal_forward_value.vp`, `causal_oracle_cycle.vp`, `history_own_later_emission.vp` and `later_value_fed_back.vp` as attacks. `Coherence::compatible` applies the same rule to its own restriction, refusing an alternate whose record installs anything at a slot this principal received (`causal_alt_route_late_leak.vp` against `causal_alt_route_early_leak.vp`). `reachable_knowledge_via` consults alternates **only** for values the primary routes leave unreachable, and `compatible` prunes only the entries it consulted: evaluating every recorded alternate on every call runs `execution_agrees` over the whole snapshot and costs three times the analysis. `validate.rs` independently re-proves `derivable` against the restriction.
...
- `state_world` reads a slot's world off the state: an attacker-installed slot
  pins itself and inherits the installed value's nonempty recorded worlds; a received slot
  ...
  The dependency walk collects every required pin before intersecting the distinct
  input world sets. Its cache confirms the state values and the input worlds it read.
  Derived-world caches retain up to eight distinct ingredient sets per known target and validate every recorded input world on a hit.
...
- `world/diagram.rs` represents world sets exactly as ordered decision diagrams. Union and intersection retain every constraint; term hashes select buckets and full equivalence confirms matches. Node sharing and bounded operation caches affect recomputation only. A derived route cannot widen an already-known value when its current worlds include an ingredient's worlds; cached inclusion facts avoid the redundant intersection, and absent facts fall back to exact diagram operations. Install validation and final joint witness grounding also require a nonempty intersection of their world constraints. Install validation intersects receive pins before ingredient worlds, discarding incompatible alternatives early.
- Unchanged unions preserve the shared `AttackerState::worlds` allocation and its epoch. Diagram operations copy edge lists only when a child or fallback changes, and intern nodes before allocating a new `Arc`. Refreshing a cached operation does not evict an unrelated entry. The unconditional-knowledge snapshot key includes the phase and vetted reuse-pair count as well as the knowledge chain, length and world epoch.
...
- **Primary routes are order-dependent**, and a read's recorded alternate routes are consulted only when primary routes *collide* at one slot (`closure_route_collision.vp`), never to retry a union the replay refused: retrying reintroduced `history_closure_incompatible.vp` even with replay equality. Alternates through a slot nobody observed are skipped (`causal_oracle_cycle.vp` at one session: a private `z2` read with no needs would excuse the cycle). The minimizer's `close_over_history` picks the first route coexisting with the mutations already chosen. Metamorphic `rotate` runs at one session, where orders agree.
...
- The gate runs inside the closure's fixed point, once per learn. Within a `try_deduction_step` the pass snapshot is fixed, so its `DeductionMemo` scope also carries `KnowledgeInputs::of_value` results (`ObtainableMemo::inputs`, keyed by the `Arc` it keeps alive) and the fallback's restricted knowledge per replay (`theory::scoped_restriction`, keyed by the replay `Arc`): both are pure in `(ps, attacker)`, and recomputing them per learn was a third of an analysis. `ctx.replayed` keeps its knowledge-keyed `Recent` groups, but the execution behind a replay only depends on the seeds, the phase and the honest halts, so `replay_execution` keeps it for the analysis and a group miss pays only `causally_grounded`. `available_before_pending` caches like `held_before` (`PENDING_BLOCKED`, `PENDING_RESTRICTED`), keyed additionally by the reading principal.
...
- `compromised_constants`: what is computable? Seed secrets leaked or sent bare at the disclosure phase: `knows private`, arguments of key-derivation primitives, including assigned private keys (`scenario_corrupt_by_leaked_derived_key.vp`), and `generates` constants only where some assignment uses them at a `secret_positions` slot (key derivation, `forgeable_secret`, `decompose.given`, derived checking key). A generated identifier or salt sent in the clear is public data (`scenario_generated_identifier.vp`). Close through `computable_from` only when **every** ingredient is `nil`, public or compromised; date an assignment at its latest ingredient (`scenario_corrupt_needs_every_ingredient.vp`).
...
Disclosure seeding follows named assignments and nested primitives only through the registry's `reveals_args` role. Exposed secret constants inherit the leak or message phase; visited constants and primitive pointers bound the walk over shared terms and aliases. Hashes, public keys and ciphertexts do not expose their private arguments through this tuple rule.
...
- `honest_run_halts` finds each creator's earliest failed check; `honest_run_unreached` closes over deliveries (`unreached_slots` / `slot_held`: a slot is computed only if its creator reached it and every received leaf was delivered by a sender that reached the send and held it) and `attacker_phase_update` publishes only computed slots whose disclosure event was reached. `attacker_absorb_disclosed` republishes, from any executed state, what it holds initially, computed and disclosed at a reached event, that the honest run withheld (`spore_ns_pk.vp`'s `m3@2`, `halted_peer_relay.vp`). `starved_slots` is the same closure over a state's foreign halts, with attacker-installed slots counted as delivered (the Lowe witness reads an installed `m2@2`) and third-party deliveries respected (`halted_peer_precomputed_relay_holds.vp`); `slot_held` credits a principal's own `knows` declaration before consulting `unreached`, since pre-existing knowledge is not something another declarer's halt can withhold, and only then falls back to `creator == who`, which a halt does withhold (`starved_by_a_later_declarer.vp`); `rule_equivalize` never resolves a name through an unreached or starved slot. Pins: `scenario_post_halt_no_oracle.vp`, `scenario_post_halt_leak_no_oracle.vp` (`c0`) against `scenario_pre_halt_oracle.vp` (`c1`).
...
- Structural term comparisons retain a bounded cache of proven equal node pairs. Weak references keep the node addresses from being recycled while a pair is cached; eviction only causes recomputation. The checked-instance bit remains part of structural identity even though term equivalence ignores it.
...
- **Three `SlotValues` must remain distinct:** `original` = honest computation/principal belief; `pre_rewrite` = mutated, unreduced; `value` = mutated, reduced. `should_use_original` / `effective_value` and `compute_visibility` choose perception.
- `Provenance`: `creator`, `sender` (`ATTACKER_ID = 0` for injection), `attacker_tainted`. Computation always follows actual rewritten inputs.
...
- **Authentication:** injective agreement, evaluated at the recipient. Fail when `provenance.sender` differs, the constant has a successful primitive use, and no interchangeable sender run emits the delivered value. `query_find_constant_usage_indices` accepts no-rewrite, successful-rewrite or unchecked uses. Acceptance is **positional**: all slots mentioning the constant must be reached, with at least one successful/unchecked use; an unrelated later failure need not cancel it. Thus a check *after* consuming the message may leave `a1`; see the completeness inventory before changing this rule.
  Sender-source filtering uses per-value binding interchangeability, so changing an unrelated scenario label does not make an authentic signature an independent forgery. Matching a sender output requires a declared send to the recipient's actor, within the current phase and reached in the re-executed sender state. A computed slot alone is insufficient. Emission memo keys include the recipient so a reached send to one actor cannot excuse an unreached send to another.
  Duplicate acceptance is checked across session and scenario copies before the matching-run exemption. Both modeled recipient instances must actually use the value successfully; query scope applies to the answering run.
...
- **Preconditions:** after finding a violation, every evaluator calls `preconditions_reached` before `attack_trace`; P must reach the send in `precondition[P -> T: d]`, and `message_available` must establish its input deliveries and computations under the relevant replay. Resolve `d` against `km`, never against the answering state: authentication answers at the recipient and freshness at the user, and both are truncated at their own halt, so a constant declared past that point would not resolve and the verdict was dropped (`precondition_independent_late_block.vp`). Confidentiality judges its gate in the execution that disclosed the value, replaying the substitutions that value's own record carries and asking the **sender's** replayed state whether it reached the send: the answering state may sit upstream of the halt and know nothing of it (`precondition_upstream_answerer.vp`). Unreached sends produce no verdict or output; probes use the same evaluator. Render the conjunct, not an annotation. `restrict` may only remove violations. `precondition_*.vp` covers all kinds, foreign halts, split recipients, multiple options, later phases and scenarios; seven `err_precondition_*.vp` pin rejection.
- **Unlinkability:** `unlink.rs::find_link_witness` seeks a pair of observable, non-attacker-authored queried values. In order: observed equality; identifying check under one participant's secret-dependent identifier (`depends_on_secret`); common secret origin (`origin_leaves`); recognized secret that can be tested but not recomputed (`recognized_secrets`, following pinned arguments' origins). These witness predicates do not decide two-world behavioral equivalence.

Unlinkability gates:

- Every witness but observed equality must `carries_shared_secret`: its value has to contain a secret leaf of an honest subterm the two queried values share, so a key the attacker forced onto both is not a common origin (`unlink_forced_key_carries_no_secret.vp`).
- `witness_shared_secret` requires a common leaf to remain held, obtainable or recomposable after **either** queried value and all its consequences are withheld (`held_independently`, `without_consequences`). Joining the queried shares is not an origin (`unlink_shares_recomposed_secret.vp`); a third held share can make it one. A queried value in the other's origins is itself a witness (`unlink_hash_chain_direct.vp`). Try check witnesses across exposed tuple components and held reconstruction ingredients too (`unlink_signature_in_tuple.vp`, `unlink_unblind_hashed_signatures.vp`). The latter links reproducible hashes of signatures through their common verification key; separate keys and undisclosed blinding factors are counterweights.
...
- Recognized candidates must not be `check_key` constructor applications: ring members would otherwise link every signature (`unlink_ringsign.vp`, `unlink_signature_links.vp`, `unlink_signature_recognized.vp`). `is_key_derivation` is structural: a `PUBKEY`-headed value names a participant, not a session.
...
`THRESHOLD_SPLIT[t](k)` models dealer Shamir sharing; `THRESHOLD_SIGN(share, nonce, commitments, message)` models RFC 9591 round-2 signing; `THRESHOLD_JOIN` interpolates shares, partials or verification shares. A `CombineRule` requires each `partial` to use a projection of `split` at `share`, agree at `agree` positions and yield `whole(secret, carry…)`; other positions may vary subject to `CombineBinding` constraints. For signing partials, each nonce's public commitment must occur in the agreed, possibly nested tuple of commitments. Rules join signing partials agreeing on commitments/message into ordinary `SIGN(k, message)`, and `PUBKEY(share)` into `PUBKEY(k)`. `SIGNVERIF` stays unchanged (`threshold_join_agreement.vp`: `e0e0e1e1`). Commitment binding in partials prevents mixing signing sessions.
...
Concrete-flight refinement of an admitted shared substitution is searched once per knowledge state. Its execution record confirms the full install signature and invalidates the cached proposals when terms, reuse evidence, derivation routes, world constraints, phase or knowledge branch change. Recalled flights are returned to ordinary validation on every visit; the cache never substitutes for validating them.
...
- `symbolic.rs`: replace controllable wires with variables and reduce as the real state does. `build_addressed` inlines every other principal's computation with the honest value (`reaches` is the owner alone), the symbolic twin of an addressed install; `solve_principal` runs it as a second Targeted pass (its own `PassKey::addressed`) for a principal with a slot delivered unguarded both to it and to someone else, where `split_delivered` also requires that other delivery to come from a **different** sender: a copy relayed through the addressed principal carries the install with it (`split_delivery_through_relay.vp`, `split_delivery_through_open_relay.vp`). Ask that of the *other* delivery only; vetoing every principal that forwards the slot anywhere withholds the pass from ordinary relays (`split_delivery_relaying_recipient.vp`), holding honest the controllable slots that are not `directly_unguarded` to it, since only a direct unguarded leg can be altered for one recipient alone. `dispose` validates that pass's proposals with `addressed = true`; `reexecute_with` stamps the flag on the install and `mutation_reaches` refuses every other principal. The minimizer reads the flag off the answering state (`Minimizer::addressed_all`) and probes with it, giving other sessions none of the shared installs. Records of an addressed execution replay as shared in `replay_diffs`, which can only refuse more. `attacker_controllable` / `Controllable` rejects guarded-unless-`mutatable_to`, self-created, wrong-phase, unused and `nil` slots. Equivalence-named slots waive unused (`equivalence_unused_received.vp`); `construct_wire_index` propagates mutability through every relay (`relay_guard_transitive.vp`, `relay_forward_before_check.vp`). DH shares become `PUBKEY($x)`.
...
- `validate.rs`: **the only search-to-query path**. Search materializes one concrete `(slot, value)` signature for dedup/validation. Validator receives no `SymbolicState` or `Substitution`; it rejects out-of-range/residual-variable/duplicate slots and independently proves installs. Required order: **controllability → derivability/causal availability → queue install → execute**; any failure abandons the whole proposal. Its independent `derivable` repeats capability exemptions (`forgeable_secret_position`, `malleable_positions`). Memoize derivability per pointer/call with scoped `DeductionMemo`; history checks stop at held terms and examine needs without tree-sized lists. Keep these predicates separate from `obtainable`: malleability differs. Forty-layer tests cover constructibility, unavailable secrets and changing capabilities.
  `remember_execution` / `recall_execution` store diffs against `execution_base` for the concrete supplied terms. A failed checked rewrite halts the corresponding principal. Compare states/signatures by `structurally_identical`, not equivalence: commuted terms narrate differently. Re-run guards on every recall; keep the single `execute_forward(` after `installs.push(`. `note_execution_closed` skips closure/queries only for current `Saturation`.

**Proposal families, in order:** unresolved-query goals; `constraint_goals`; blanket substitution; single-slot substitutions; then Constructed-only sibling flights and held-protocol/required-rewrite candidates, alone and with blanket. A proposal `validate` refuses is retried as its `admitted_prefix`, the installs that already pass the per-install gates, only when it is an `oracle_chain`: every dropped install is what this principal itself emits under the kept ones (`emissions_under`), so a chain whose later install is derivable only after the earlier one runs is proposed in two rounds (`solver_oracle_input_prefix.vp`). Unrestricted, the fallback made `examples/messaging/userbase.vp` ten times slower and changed its witness. `oracle_input_goals` (Targeted) unifies each of this principal's symbolic emissions with the shapes every principal's checks demand (`build_rewrite_shapes_with` over each check in that principal's own symbolic state, both sides of an `ASSERT`), binds the check's own slot to the emission when it belongs to this principal, and grounds other principals' receive variables by their honest alignment (attacker public key or `nil`); an occurs check keeps a slot from being bound in terms of itself (`replay_pump_reflection.vp` overflowed the stack without it). The shape set also carries each demanded shape's own primitive arguments, kept only where some emission has that head and arity, which is what lets two oracles be walked in series (`solver_oracle_chained_inputs.vp`, `…_separated.vp`). Pins: `solver_oracle_input_other_principal.vp`, `solver_oracle_input_salt.vp`, `solver_oracle_input_certifies_key.vp` against `…_domain_separated.vp`. Explicit honest-other-slot variants are redundant after `leave_honest_slots`. `fill_aligned_with` adds `keyed_free`, `preserved_free` and, only for outstanding equivalence, `distinguish`. Shared `aligned_free_positions` walks proposed/honest pairs iteratively, skips ground branches and visits repeated pairs once (`shared_transcript_replay.vp`).
...
`witness.rs::minimize_witness` tries candidates in each principal's pristine session: canonical DH MitM, target-specific world support, recorded mutations, then those with keys replaced by `PUBKEY(nil)`; retain a resolving candidate and drop installs individually. Probes use a scratch context reseeded from the **passive baseline**, not final knowledge. Every candidate, including an empty install set, is replayed in that scratch context. A candidate that cannot be reproduced or grounded leaves the query unresolved and marks its witness-validation truncation; it is never published as an attack.

- `forged_from` asks failed checks for concrete constructor shapes at controlled non-key slots. `payload_shapes` tries held honest fields and attacker public keys before `nil`; every candidate is independently replayed.
- `forged_flight` ranks candidates by `(halts, stuck)` and supports both non-increasing and strictly improving searches. Solver refinement uses strict improvement; all proposed wire values still pass independent derivability, causality, depth, control and world checks.
- Candidate scoring reuses the complete failure list already produced by re-execution, including checks beyond the eventual halt, and retains a score until the installs change. Repeated installs at the same slot keep the separate check-collection path because sequential authorship decisions can differ. Search ordering, candidate admission and the final independent witness replay are unchanged.
- **The forged-flight search is where the time goes**, and `dispose` runs it for every proposal that ran, on every round, because its flights are recalled per knowledge state. Everything it scores is knowledge-independent: `reexecute_with` reads nothing from the attacker, so `VerifyContext::staged_costs` / `staged_checks` memoise a pristine session's `(halted, failing wanted slots)` and its failing checks per `(principal, installs)` for the whole analysis, and `prime_costs` re-executes one slot's uncached trials in parallel while every memo lookup stays on the caller thread. Two prunings are exact: a slot the attacker cannot install into is never tried, since `admitted` would drop every trial there; and in strict-improvement mode only slots that reach a currently failing wanted check (declaration cone plus the constants installed values mention) are tried, since a trial anywhere else leaves every failing check failing and cannot lower `(halts, stuck)`. One `DeductionMemo` scope spans a search so `validator_admits` shares subproofs. Unmemoised, `aws_nitro_attestation.vp` went from 8s in 1.5.0 to over 300s and `junglegym_threshold_ring.vp` from 2.5s to 43s at one session.
- `seeded_mutations` retains the actual sender execution of a forwarded value, even when a weakening assumption also makes that value independently constructible. `forgeable_forwarding_retains_its_sender_execution` pins this at both session counts.
- `probe_with` evaluates its query on the same state it returns for narration. It grounds installs greedily: earlier pending receives constrain only the **same recipient's run**; every chosen install must be `attacker_can_derive`-able from preceding knowledge. Clones run concurrently. Joint pending-receive causality is described above. `choose` prefers grounded, nonhalting witnesses; `drop_one` preserves groundedness (`the_lowe_witness_replays_through_bob_before_it_learns_the_nonce`, `spore_otway_rees.vp`).
- `seeded_mutations` breadth-first closes tainted diffs over their values' histories, skipping `costs_nothing`. If no grounded witness is found, retry with each `Obtained` or `Leaked` read's recorded diffs restricted to its creator's `reach_cone`, including checks before emission: unrelated later receives are not prerequisites for the read. For derived values, the retry follows `ingredients()` instead of importing their ambient record's diffs; assembling or opening a tuple adds no execution prerequisites beyond its inputs. The retry passes the same replay and grounding checks; already validated witnesses keep their search order. `session_key_divergence_late_confirmation.vp` and `session_key_divergence_bundled.vp` are `e0` at one session and `e1` at two; their guarded companions stay `e0`. Bound slots by `km.slots.len()`, not possibly truncated `ps.values.len()` (`spore_ns_pk.vp`).
...
- **Words require evidence:** only `session_prefix`, reading `MutationRecord.principal_id`, names a derivation's run; never call another run "earlier". Only `copy_sibling_replay` justifies "from another session" in `Step::Replay`; otherwise say "unaltered". `Obtained` is "on the wire" only with `sent_by`.

### The soundness tests (query.rs::tcb_tests)

The paper audits the result-writing path separately from the conditional witness-transfer theorem. These tests pin source-level boundaries; they do not prove the evaluator predicates or single-history reachability:
...
| (vii) | a value entering a state by forwarding is exactly what its sender sent: over a declared leg, in a reached phase, from a run that reached the send, preserving its creator and upstream attacker origin while setting taint | `a_forwarded_value_is_only_ever_what_its_sender_sent` |
...
1.5.1 made several models that 1.5.0 finished in seconds run for minutes or hours, without changing a verdict: the concrete forged-flight search (`dispose` → `forged_check_flights`) re-executed the principal for every candidate shape of every proposal on every round. Measured single-threaded on the same machine, 1.5.0 → 1.5.1 → after this audit: `aws_nitro_attestation.vp` 8s → >300s → 24s; `junglegym_threshold_ring.vp` (one session) 2.5s → 43s → 6s; `closure_halted_send.vp` 6.3s → 25s → 5.5s; `simplex_shortlink.vp` 0.1s → 3.6s → 0.7s. Every change is a memo or an exact pruning: full-output corpus diffs at one and two sessions are byte-identical and the suite passes. What remains above 1.5.0 is the search's distinct trials, each a full re-execution; making re-execution incremental would be the next step and is a different engine change.

The method that found it: build both releases with symbols (`CARGO_PROFILE_RELEASE_STRIP=false CARGO_PROFILE_RELEASE_DEBUG=1`), `sample` the single-threaded run (`VERIFPAL_THREADS=1`), aggregate inclusive counts per function, and count calls with temporary counters in both trees, since a profile cannot tell more calls from slower calls. Memo keys must be what a function reads, not what happens to be at hand: a knowledge-chain key on a knowledge-independent computation (`replay_diffs`, `execution_agrees`, `reexecute_with`) is a cache that never hits.
...
| `unlink::attacker_authored_slot` refusal | gate | 5 models (historical) | soundness guard; retained examples include `unlinkability.vp`, `unlink_injected_equality.vp`, `session_unlink_stable.vp`, `precondition_unlink_halt.vp` |
