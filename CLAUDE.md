...
- `world/diagram.rs` uses **exact** ordered decision diagrams. Union/intersection retain all constraints; hashes select buckets, full equivalence confirms matches. Bounded caches change recomputation only. Inclusion facts avoid redundant work; missing facts fall back to exact operations. Never cap alternatives.
- Install validation and final joint witness grounding require nonempty world intersections. Intersect receive pins before ingredient worlds. `worlds` parallels `known`; `worlds_epoch` invalidates memos, and unchanged unions preserve allocation/epoch. Unconditional-knowledge keys include phase, vetted-pair count, knowledge chain/length and world epoch.
...
Memo keys must include exactly what the computation reads. A knowledge-chain key on knowledge-independent `replay_diffs`, `execution_agrees` or `reexecute_with` prevents useful hits. Profile release binaries with `CARGO_PROFILE_RELEASE_STRIP=false CARGO_PROFILE_RELEASE_DEBUG=1`, `VERIFPAL_THREADS=1`, and `sample`; temporary call counters distinguish more calls from slower calls. Keep full-output comparisons at both session counts when optimizing.

The September 2026 audit traced a major 1.5.1 regression to forged-flight scoring on every proposal/round. Analysis-lifetime execution memos and exact pruning reduced it; remaining distinct trials still require full re-execution. **Score one slot's trials sequentially**: thread-pool scoring rebuilt thread-local rewrite/equality caches, doubled CPU time and regressed two-session performance.

A separate unresolved cost is exact world-set arithmetic. Two-session `simplex_queue_rotation_retained.vp` in the SimpleX repository remains infeasible: measured worlds had 119k nodes, 2.2M edges and 35 billion alternatives; 87% of time was diagram arithmetic. Changing variable order did not help. The older collapse beyond sixteen alternatives is **not an acceptable fix**. `widen` checks remembered inclusions before exact traversal.

Two measured experiments were left out of the tree; do not repeat them without a new reason. Incremental products from cached world deltas preserved corpus output but provided no benefit and cost 13% on two-session `junglegym_threshold_ring.vp` (2.3× with true deltas). Factoring worlds into receive pins and per-check pass sets admitted more executions but enlarged diagrams and exceeded 900s on that model. It also required distinguishing **computed** from **emitted** worlds for duplicate acceptance. Both left SimpleX infeasible. Lazy check evaluation with worlds as pins plus pending checks is an unimplemented engine change, not a demonstrated optimization.
